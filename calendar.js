(function drawCalendar(
    ...
    const longestRow = year.reduce((result, current) => {
      const cellsBeforeFirstDay =
        weekStartsOnMonday && current.firstDay === 0
          ? 6
    ...
    function drawDaysOfWeek(totalCellsInEachRow, weekStartsOnMonday) {
      ...
      const endDay = weekStartsOnMonday
        ? totalCellsInEachRow + 1
        : totalCellsInEachRow;
    ...
    function drawRow(month, totalCellsInEachRow, crossOutPastDays) {
      ...
      for (let i = 0; i < totalCellsInEachRow; i++) {
        ...
        console.log(month.startCell, month.endCell); 

        // simulate booked days

        for (let j=0; j<10; j++){
            const random = Math.floor(Math.random()*(26 - 6)+6);
            if (currentDay == random){
